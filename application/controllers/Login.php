...
class Login extends CI_Controller {
    ...
    private function persist_session_cookie() {
        setcookie(
            $this->config->item('sess_cookie_name'),
            session_id(),
            ($this->config->item('sess_expiration') > 0 ? time() + (int) $this->config->item('sess_expiration') : 0),
...
//        exit($path);
    ...
    function do_login() {
        ...
        $this->session_debug('do_login_before_validate', array('email' => $email));

        $login_status = $this->validate_login($email, $password);
        $this->session_debug('do_login_after_validate', array('login_status' => $login_status));

        if ($login_status == 'success') {
            $this->session_debug('do_login_before_write_close', array('login_type' => $this->session->userdata('login_type')));
            $this->persist_session_cookie();
            $this->session_debug('do_login_after_setcookie', array('headers' => headers_list()));
            session_write_close();
            ...
            if ($this->session->userdata('admin_login') == 1) {
                ...
                redirect(base_url() . 'index.php?parents/dashboard');
            } elseif ($this->session->userdata('librarian_login') == 1) {
                redirect(base_url() . 'index.php?librarian/dashboard');
            } elseif ($this->session->userdata('accountant_login') == 1) {
                redirect(base_url() . 'index.php?accountant/dashboard');
            } elseif ($this->session->userdata('hostel_login') == 1) {
                redirect(base_url() . 'index.php?hostel/dashboard');
            }
        }
    ...
    function ajax_login() {
        ...
        if ($login_status == 'success') {
            $this->session->sess_regenerate(TRUE); // 🔥 important
            session_write_close(); // 🔥 force save

            //echo '<pre>';
            //print_r($this->session->userdata());
            //die;
            // Check what type of user logged in and set appropriate redirect
            if ($this->session->userdata('admin_login') == 1) {
                $response['redirect_url'] = base_url() . 'index.php?admin/dashboard';
            } elseif ($this->session->userdata('teacher_login') == 1) {
                $response['redirect_url'] = base_url() . 'index.php?teacher/dashboard';
            } elseif ($this->session->userdata('student_login') == 1) {
                $response['redirect_url'] = base_url() . 'index.php?student/dashboard';
            } elseif ($this->session->userdata('parent_login') == 1) {
                $response['redirect_url'] = base_url() . 'index.php?parents/dashboard';
            } else {
    ...
    function validate_login($email = '', $password = '') {
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->session_debug('validate_login_admin_success');
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->session_debug('validate_login_teacher_success');
            return 'success';
        }
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->session_debug('validate_login_student_success');
            return 'success';
        }
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->session_debug('validate_login_parent_success');
            return 'success';
        }
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->session->set_userdata(array(
                ...
                'login_user_id'   => $row->librarian_id,
                'name'            => $row->name,
                'login_type'      => 'librarian'
            ));
            $this->session_debug('validate_login_librarian_success');
            return 'success';
        }
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->session->set_userdata(array(
                ...
                'login_user_id'    => $row->accountant_id,
                'name'             => $row->name,
                'login_type'       => 'accountant'
            ));
            $this->session_debug('validate_login_accountant_success');
            return 'success';
        }
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->session->set_userdata(array(
                ...
                'login_user_id' => $row->hostel_id,
                'name'          => $row->name,
                'login_type'    => 'hostel'
            ));
            $this->session_debug('validate_login_hostel_success');
            return 'success';
        }

        $this->session_debug('validate_login_invalid');
        return 'invalid';
    }
